...
/*L6480 コントロール　コマンド
 ...
 L6480_getstatus(); //statusレジスタの値を返す （L6480_getparam_status();と同じ）
 ...
 //ありませんL6480_spped //[R] 現在速度read onry  (20bit)
 ...
 L6480_setparam_minspeed(val); //[R, WS]最小速度default 0x000 (1+12bit) (0.238*val+[step/s])
 ...
 //ありませんL6480_adcout //[R] read onry (5bit) ADCによる逆起電力補償の大きさかな？
 L6480_setparam_ocdth(val); //[R, WR]過電流しきい値default 0x8 (4bit) (375*val+375[mA])
 L6480_setparam_stallth(val); //[R, WR]失速電流しきい値？default 0x40 (7bit) (31.25*val+31.25[mA])
 L6480_setparam_stepmood(val); //[R, WH]ステップモードdefault 0x07 (1+3+1+3bit)
 L6480_setparam_alareen(val); //[R, WS]有効アラームdefault 0xff (1+1+1+1+1+1+1+1bit)
 L6480_setparam_config(val); //[R, WH]各種設定default 0x2e88 (3+3+2+1+1+1+1+1+3bit)
 //L6480_status //[R]状態read onry (16bit)
...
void L6480_setparam_speed(long val){L6480_transfer(0x04,3,val);}
...
void L6480_setparam_status(long val){L6480_transfer(0x1B,2,val);}
...
long L6480_setparam_gatecfg1(){return L6480_getparam(0x18,2);}
long L6480_setparam_gatecfg2(){return L6480_getparam(0x19,1);}
long L6480_setparam_config(){return L6480_getparam(0x1A,2);}
long L6480_setparam_status(){return L6480_getparam(0x1B,2);}
